...
import { mockPlayers, mockCoaches } from '../data/mockData';
import {
  User as UserIcon,
  Flame,
  X,
...
export const RosterSection: React.FC<RosterSectionProps> = ({ currentRole = 'SUPER_ADMIN' }) => {
  ...
  const handleOpenAddModal = () => {
    ...
    setFormData({
      ...
      age: 21,
      photo: 'https://images.unsplash.com/photo-1546519638-68e109498ffc?w=600&auto=format&fit=crop&q=80',
      bio: 'Joueur talentueux et déterminé, prêt à enflammer les parquets de FIRE STONE.',
      experienceYears: 3,
      ppg: 16.5,
      rpg: 4.8,
      apg: 5.2,
      ...
      ftPct: 82.0,
      efficiency: 20.0,
      achievementsStr: 'Recrue Pro FIRE STONE',
      shooting: 85,
      passing: 84,
      defense: 82,
      athleticism: 86,
  ...
  const handleOpenEditModal = (player: Player, e: React.MouseEvent) => {
    ...
    setFormData({
      ...
      spg: player.seasonStats.spg,
      bpg: player.seasonStats.bpg,
      fgPct: player.seasonStats.fgPct,
      threePtPct: player.seasonStats.threePtPct,
      ftPct: player.seasonStats.ftPct,
      efficiency: player.seasonStats.efficiency,
      achievementsStr: player.achievements.join(', '),
      shooting: player.skillsRadar.shooting,
      passing: player.skillsRadar.passing,
      defense: player.skillsRadar.defense,
      athleticism: player.skillsRadar.athleticism,
      iq: player.skillsRadar.iq,
      rebounding: player.skillsRadar.rebounding
    });
    setShowAdminModal(true);
  };
  ...
  const handleSavePlayer = (e: React.FormEvent) => {
    ...
    if (editingPlayerId) {
      ...
      const updatedList = playersList.map((p) => {
        if (p.id === editingPlayerId) {
          const updated: Player = {
            ...
            bio: formData.bio || 'Joueur membre de la grande famille FIRE STONE.',
      ...
      const newPlayer: Player = {
        ...
        age: formData.age,
        photo: formData.photo,
        bio: formData.bio || 'Nouveau joueur intégré au club FIRE STONE.',
        experienceYears: formData.experienceYears,
        seasonStats: {
          ppg: formData.ppg,
          rpg: formData.rpg,
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {isAuthorized && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#B91C1C] to-[#881337] text-white font-bold text-xs shadow-md hover:scale-105 transition-all self-start md:self-auto"
          >
      ...
      {(positionFilter === 'TOUS' || positionFilter === 'ENTRAÎNEURS') && (
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-2">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <Shield className="w-5 h-5 text-[#D97706]" /> Staff Technique & Entraîneurs
            </h3>
            <span className="text-xs text-slate-400 font-semibold">{mockCoaches.length} Membres Staff</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {mockCoaches.map((coach) => (
              <div key={coach.id} className="glass-panel p-6 rounded-3xl border border-white/10 space-y-4 bg-[#0A0C13]">
                <div className="flex items-center gap-4">
                  ...
                  />
                  <div>
                    <h4 className="text-base font-extrabold text-white">{coach.name}</h4>
                    <span className="text-xs text-[#D97706] font-bold block">{coach.roleTitle}</span>
                    <span className="text-[11px] text-slate-400">{coach.experience}</span>
                  </div>
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">{coach.bio}</p>
                <div className="pt-2 border-t border-white/10 flex items-center justify-between text-xs">
                  <span className="text-slate-400">Taux de Victoires : <strong className="text-emerald-400">{coach.winRate}</strong></span>
                  <span className="text-slate-400">Spécialité : <strong className="text-white">{coach.specialty}</strong></span>
                </div>
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div className="relative rounded-3xl bg-[#0F111A] border border-white/15 p-4 pt-0 transition-all duration-500 transform group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:shadow-red-950/60 group-hover:border-[#B91C1C]/60 overflow-visible">
                  ...
                  <div className="relative h-60 w-full overflow-hidden flex justify-center items-end my-2 rounded-2xl">
                    ...
                    <div className="absolute bottom-2 inset-x-2 h-44 rounded-2xl bg-gradient-to-t from-[#B91C1C]/40 via-amber-950/20 to-transparent border border-white/5 group-hover:from-[#B91C1C]/70 transition-all duration-500" />
                  ...
                  <div className="relative z-30 pt-2 space-y-2 border-t border-white/10">
                    ...
                    <div className="grid grid-cols-3 gap-1 bg-black/50 p-2 rounded-xl border border-white/5 text-center text-xs">
                      <div>
                        ...
                        <span className="font-black text-white">{player.seasonStats.ppg}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 block uppercase">REB</span>
                        <span className="font-black text-white">{player.seasonStats.rpg}</span>
                      </div>
                      <div>
                        <span className="text-[9px] text-slate-400 block uppercase">AST</span>
                        <span className="font-black text-[#D97706]">{player.seasonStats.apg}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-black/50 p-4 rounded-2xl border border-white/10 space-y-2">
                  ...
                  <div className="grid grid-cols-5 gap-2 text-center text-xs pt-1">
                    <div>
                      ...
                      <div className="font-black text-white text-base">{selectedPlayer.seasonStats.ppg}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400">RPG</div>
                      <div className="font-black text-white text-base">{selectedPlayer.seasonStats.rpg}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400">APG</div>
                      <div className="font-black text-white text-base">{selectedPlayer.seasonStats.apg}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400">FG%</div>
                      <div className="font-black text-emerald-400 text-base">{selectedPlayer.seasonStats.fgPct}%</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-slate-400">3PT%</div>
                      <div className="font-black text-[#D97706] text-base">{selectedPlayer.seasonStats.threePtPct}%</div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="h-52 w-full flex items-center justify-center">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="70%" data={[
                        { subject: 'Tir', A: selectedPlayer.skillsRadar.shooting },
                        { subject: 'Passe', A: selectedPlayer.skillsRadar.passing },
                        { subject: 'Défense', A: selectedPlayer.skillsRadar.defense },
                        { subject: 'Athlétisme', A: selectedPlayer.skillsRadar.athleticism },
                        { subject: 'QI', A: selectedPlayer.skillsRadar.iq },
                        { subject: 'Rebond', A: selectedPlayer.skillsRadar.rebounding },
                      ]}>
                        ...
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <button
                              ...
                              className="mt-1.5 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-violet-700 to-violet-900 hover:from-violet-600 hover:to-violet-800 text-white font-bold text-[10px] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
