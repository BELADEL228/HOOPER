...
export const SponsorDonationPortal: React.FC = () => {
  ...
  const [sponsors, setSponsors] = useState<Array<{ id: string; name: string; logo: string; tier: 'PLATINE' | 'OR' | 'ARGENT'; description: string; website: string; contribution: string }>>([]);
  const [partnershipMessage, setPartnershipMessage] = useState('');
  const [partnershipForm, setPartnershipForm] = useState({ companyName: '', teamId: '', type: 'SPONSORING', title: '', message: '', budget: '' });
  const [teams, setTeams] = useState<Array<{ id: string; name: string }>>([]);

  useEffect(() => {
    fetch(apiUrl('/sponsors')).then(async (response) => { if (response.ok) { const remote = await response.json() as Array<{ id: string; companyName: string; logoUrl?: string | null; description?: string | null; website?: string | null }>; setSponsors(remote.map((sponsor) => ({ id: sponsor.id, name: sponsor.companyName, logo: sponsor.logoUrl || '🏢', tier: 'OR' as const, description: sponsor.description || '', website: sponsor.website || '', contribution: 'Partenaire vérifié' }))); } }).catch(() => undefined);
    fetch(apiUrl('/teams')).then(async (response) => { if (response.ok) setTeams(await response.json()); }).catch(() => undefined);
  }, []);
  ...
  const handlePartnershipSubmit = async (event: React.FormEvent) => {
    ...
    if (!session?.token) { setPartnershipMessage('Connectez-vous pour envoyer une demande de partenariat.'); return; }
    const response = await fetch(apiUrl('/sponsorship-requests'), { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.token}` }, body: JSON.stringify({ ...partnershipForm, budget: partnershipForm.budget ? Number(partnershipForm.budget) : undefined }) });
    const data = await response.json();
    setPartnershipMessage(response.ok ? 'Demande de partenariat envoyée.' : data?.error || 'Demande impossible.');
  };
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <form onSubmit={handlePartnershipSubmit} className="glass-panel p-6 rounded-3xl border border-[#D97706]/30 space-y-4">
        <div><h3 className="text-xl font-bold text-white">Proposer un partenariat</h3><p className="text-xs text-slate-400">Présentez votre entreprise et votre opportunité au club.</p></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input required placeholder="Nom de l’entreprise" value={partnershipForm.companyName} onChange={(e) => setPartnershipForm({ ...partnershipForm, companyName: e.target.value })} className="glass-input rounded-xl p-3 text-xs" />
          <select required value={partnershipForm.teamId} onChange={(e) => setPartnershipForm({ ...partnershipForm, teamId: e.target.value })} className="glass-input rounded-xl p-3 text-xs"><option value="">Équipe bénéficiaire</option>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select>
          <input required placeholder="Titre de la proposition" value={partnershipForm.title} onChange={(e) => setPartnershipForm({ ...partnershipForm, title: e.target.value })} className="glass-input rounded-xl p-3 text-xs" />
          <input type="number" min="0" placeholder="Budget indicatif (FCFA)" value={partnershipForm.budget} onChange={(e) => setPartnershipForm({ ...partnershipForm, budget: e.target.value })} className="glass-input rounded-xl p-3 text-xs" />
          <textarea required placeholder="Décrivez votre proposition" value={partnershipForm.message} onChange={(e) => setPartnershipForm({ ...partnershipForm, message: e.target.value })} className="glass-input min-h-20 rounded-xl p-3 text-xs md:col-span-2" />
        </div>
        {partnershipMessage && <p className="text-xs text-amber-300">{partnershipMessage}</p>}
        <button type="submit" className="rounded-xl bg-[#D97706] px-4 py-2.5 text-xs font-black text-black">Envoyer la proposition</button>
      </form>
        ...
        <div className="lg:col-span-7 glass-panel p-6 sm:p-8 rounded-3xl border border-white/15 space-y-6">
          ...
          {!isSubmitted ? (
            <form onSubmit={handleDonationSubmit} className="space-y-5 text-xs">
              ...
              <button
                type="submit"
                className="w-full py-3.5 rounded-xl bg-linear-to-r from-[#FF2A3B] to-[#E60023] text-white font-black text-sm shadow-lg shadow-red-500/30 hover:scale-[1.01] transition-all"
              >
